...
with st.sidebar:
    ...
    sort_col = st.selectbox("Sortera tabell efter", ["modified_time_utc","start_time_utc","county_name","message_type","road_number"])
...
if g.empty or g["count"].sum() == 0:
    ...
    fig.update_layout(
        title=f"{'Alla län' if show_all else 'Topp 10 län med flest händelser'}",
        yaxis=dict(type="category", categoryorder="array", categoryarray=labels_disp, title=""),
        xaxis=dict(type="linear", rangemode="tozero", title="Antal händelser", range=[0, max(1, int(max_count * 1.15))]),
...
if approx_missing and not m.empty:
    m["latitude"] = m.apply(
        lambda r: r["latitude"] if pd.notna(r["latitude"]) else COUNTY_CENTER.get(r["county_name"], (None, None))[0], axis=1
    )
    m["longitude"] = m.apply(
        lambda r: r["longitude"] if pd.notna(r["longitude"]) else COUNTY_CENTER.get(r["county_name"], (None, None))[1], axis=1
    )
...
if map_df.empty:
    ...
    # Tooltip-kolumner (rena strängar)
    ...
    if use_county_colors and "county_colors" in st.session_state:
        map_df["__color_rgba__"] = map_df["county_name"].map(st.session_state.county_colors).apply(lambda c: hex_to_rgba(c, 210))
    else:
        map_df["__color_rgba__"] = [[230, 57, 70, 210]] * len(map_df)

    # Auto-zoom mot klickade län
    ...
    # LAGER (viktigt: records istället för DataFrame)
...
st.dataframe(
    f_sorted[["incident_id","message_type","status","county_name","road_number","location_descriptor","start_time_utc","end_time_utc","modified_time_utc","latitude","longitude"]] if not f_sorted.empty else f_sorted,
    use_container_width=True,
)
...
if not f.empty and "message_type" in f.columns:
    ...
    fig_types = px.bar(type_counts, x="Antal", y="Typ", orientation="h", text="Antal", title="Fördelning av händelsetyper")
